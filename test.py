...
async def save_pt_content(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ၁။ ပုံ ပါ/မပါ စစ်ဆေးခြင်း
    if msg.photo:
        photo_file = await msg.photo[-1].get_file()
        photo_bytes = await photo_file.download_as_bytearray()
        content_to_ai.append({"mime_type": "image/jpeg", "data": bytes(photo_bytes)})
        # ပုံမှာ Caption (စာ) ပါရင် ထည့်ပေါင်းမယ်
        if msg.caption:
            content_to_ai.append(msg.caption)
    
    # ၂။ စာသားသက်သက်ဆိုလျှင်
    elif msg.text:
    ...
    # AI ကို နေရာအလိုက် စျေးနှုန်းရှာခိုင်းမည့် Prompt
    prompt = """
    Extract price data from this content. 
    If it's an image, look for numbers next to currency labels (ks, kyats, etc.).
    Return JSON: {'category': 'game'/'premium', 'items': {'qty': price}, 'main_item': 'name'}.
    """
    ...
    try:
        ...
        await msg.reply_text("❌ AI မှ စျေးနှုန်းကို ရှာမတွေ့ပါ။ စာသား/ပုံကို ပြန်စစ်ပေးပါဗျ။")
        return WAIT_PT_CONTENT

    # ၃။ Admin ကို မူရင်းအတိုင်း Preview ပြန်ပြခြင်း (ပုံဆိုပုံ၊ စာဆိုစာ)
    ...
    preview_text = (f"📊 **AI ခွဲခြားရရှိမှု**\n"
                    f"အမျိုးအစား: {parsed_data['category']}\n"
                    f"ပစ္စည်း: {parsed_data['main_item']}\n"
                    f"နမူနာ: {list(parsed_data['items'].items())[:3]}")
    
    kb = [[InlineKeyboardButton("✅ အတည်ပြုသည်", callback_data="confirm_pt_save")],
          [InlineKeyboardButton("❌ ပြန်ပြင်မည်", callback_data="act_edit")]]

    # မူရင်းပို့ထားတဲ့ Message ကို ပြန်ပြ (Copy) လုပ်မယ်
