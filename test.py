...
async def save_pt_content(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    parsed_data = await parse_pt_logic(msg.text)
    
    if not parsed_data:
        await msg.reply_text("❌ AI မှ အချက်အလက် ခွဲခြားမရပါ။ စာသားကို ပြန်စစ်ပေးပါရှင်။")
        return WAIT_PT_CONTENT

    # ယာယီ သိမ်းထားခြင်း (Confirmation ရမှ Database ထဲ သွားသိမ်းမည်)
    context.user_data['temp_parsed'] = parsed_data
    context.user_data['temp_msg_id'] = msg.message_id
    context.user_data['temp_chat_id'] = msg.chat_id

    preview = (f"🔍 **PT Preview**\n"
               f"အမျိုးအစား: {parsed_data.get('category', 'N/A')}\n"
               f"ပစ္စည်း: {parsed_data.get('main_item', 'N/A')}\n"
               f"နမူနာစျေးနှုန်း: {list(parsed_data.get('items', {}).items())[:3]}\n\n"
               f"ဤအချက်အလက်များ မှန်ကန်ပါသလားရှင်?")
    
    kb = [[InlineKeyboardButton("✅ ဟုတ်တယ် (အတည်ပြုမည်)", callback_data="confirm_pt_save")],
          [InlineKeyboardButton("❌ မဟုတ်ဘူး (ပြန်ပို့မည်)", callback_data="act_edit")]]
    
    await msg.reply_text(preview, reply_markup=InlineKeyboardMarkup(kb))
    return CONFIRM_PT_SAVE # State အသစ်တစ်ခု Define လုပ်ပေးရန်လိုသည်
