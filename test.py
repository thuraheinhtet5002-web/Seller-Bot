...
async def parse_pt_logic(text):
    prompt = f"""
    Convert this price list into a JSON object with these keys:
    1. "category": Identify if this list is for "game" (UC/Diamonds/Gold) or "premium" (App Subscriptions like Telegram/Spotify).
    2. "items": A dictionary of {{"quantity": price}}. Rule: Last number is the price.
    3. "main_item": The name of the item (e.g., "PUBG UC", "Spotify Premium").
...
# Game Item များအတွက် ၃၀,၀၀၀ အထိ ပေါင်းလဒ် ကြိုတွက်ပေးသည့် Helper
def precalculate_game_sums(items_dict, limit=30000):
    sorted_qtys = sorted([int(k) for k in items_dict.keys()])
    if not sorted_qtys: return {}
    dp = {0: 0}
    for i in range(1, limit + max(sorted_qtys) + 1):
        for qty in sorted_qtys:
            if i >= qty and (i - qty) in dp:
                price = dp[i - qty] + items_dict[str(qty)]
                if i not in dp or price < dp[i]:
                    dp[i] = price
    return {str(k): v for k, v in dp.items() if k > 0}

async def save_pt_content(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = update.message
    content_to_ai = []
    
    # ၁။ Photo သို့မဟုတ် Text ကို Content အဖြစ် ပြင်ဆင်ခြင်း
    if msg.photo:
        photo_file = await msg.photo[-1].get_file()
        photo_bytes = await photo_file.download_as_bytearray()
        content_to_ai.append({"mime_type": "image/jpeg", "data": bytes(photo_bytes)})
        if msg.caption: content_to_ai.append(msg.caption)
    elif msg.text:
        content_to_ai.append(msg.text)

    if not content_to_ai:
        await msg.reply_text("⚠️ စျေးနှုန်းစာသား သို့မဟုတ် ပုံကို ပို့ပေးပါဗျ။")
        return WAIT_PT_CONTENT

    # AI Prompt: Premium Emojis များကို ကျော်သွားရန် ညွှန်ကြားခြင်း
    prompt = """
    Extract price data from this list. 
    IMPORTANT: Ignore all decorative emojis (including Telegram Premium ones), 💎 icons, and symbols. 
    Focus only on the numerical relationship between Quantity and Price.
    Return ONLY a clean JSON object:
    {
      'category': 'game' or 'premium',
      'main_item': 'Item Name',
      'items': { "quantity_text": price_integer }
    }
    """
    
    try:
        response = await model.generate_content_async([prompt] + content_to_ai)
        # JSON Block ကို Regex ဖြင့် ဆွဲထုတ်ခြင်း (ပိုမို ခိုင်မာစေရန်)
        json_match = re.search(r'\{.*\}', response.text, re.DOTALL)
        if not json_match: raise ValueError("No JSON found")
        
        parsed_data = json.loads(json_match.group(0))

        # ၂။ Admin ကို Preview ပြန်ပြခြင်း (မူရင်း Premium Emoji များအတိုင်း ပြပါမည်)
        context.user_data.update({'temp_parsed': parsed_data, 'temp_msg_id': msg.message_id, 'temp_chat_id': msg.chat_id})
        
        preview_text = (f"📊 **AI ခွဲခြားရရှိမှု**\n"
                        f"အမျိုးအစား: {parsed_data.get('category')}\n"
                        f"ပစ္စည်း: {parsed_data.get('main_item')}\n"
                        f"စျေးနှုန်းအရေအတွက်: {len(parsed_data.get('items', {}))} ခု")
        
        kb = [[InlineKeyboardButton("✅ အတည်ပြုသည်", callback_data="confirm_pt_save")],
              [InlineKeyboardButton("❌ ပြန်ပြင်မည်", callback_data="act_edit")]]

        # copy_message သည် Premium Emojis များကို မူရင်းအတိုင်း ထိန်းသိမ်းပေးပါသည်
        await context.bot.copy_message(chat_id=msg.chat_id, from_chat_id=msg.chat_id, message_id=msg.message_id)
        await msg.reply_text(preview_text + "\n\nအပေါ်က အချက်အလက်တွေ မှန်ကန်ပါသလားဗျ?", reply_markup=InlineKeyboardMarkup(kb))
        return CONFIRM_PT_SAVE

    except Exception as e:
        logging.error(f"AI Parse Error: {e}")
        await msg.reply_text(f"❌ AI မှ စျေးနှုန်းကို ရှာမတွေ့ပါ။ စာသား/ပုံကို ပြန်စစ်ပေးပါဗျ။")
        return WAIT_PT_CONTENT
    
async def confirm_pt_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    btn_id = context.user_data.get('target_btn_id')
    p_data = context.user_data.get('temp_parsed')
    
    # Selective Reset Logic: Category ပြောင်းသွားမှ Cache ကို ဖျက်မည်
    old_res = db_query("SELECT product_type FROM buttons WHERE id=%s", (btn_id,), fetch=True)
    if old_res and old_res[0][0] != p_data['category']:
        db_query("DELETE FROM learning_cache WHERE btn_id=%s", (btn_id,))

    # Game ဖြစ်ပါက ၃၀,၀၀၀ အထိ ပေါင်းလဒ်တွက်မည်
    final_data = p_data['items']
    if p_data['category'] == "game":
        final_data = precalculate_game_sums(p_data['items'])

    db_query("UPDATE buttons SET content_msg_id=%s, content_chat_id=%s, parsed_data=%s, product_type=%s WHERE id=%s", 
             (context.user_data['temp_msg_id'], context.user_data['temp_chat_id'], json.dumps(final_data), p_data['category'], btn_id))
    
    await query.edit_message_text("✅ အချက်အလက်များ အားလုံးကို (၃၀,၀၀၀ အထိ ပေါင်းလဒ်များ အပါအဝင်) သိမ်းဆည်းပြီးပါပြီ။")
...
async def start_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['order_btn_id'] = btn_id
    
    # အမျိုးအစားကို စစ်ဆေးခြင်း
    res = db_query("SELECT product_type, name FROM buttons WHERE id=%s", (btn_id,), fetch=True)
    p_type, b_name = res[0]

    # AI Learning မှတစ်ဆင့် တိုက်ရိုက်လာခဲ့လျှင် (ဥပမာ- "Ep" ဟု ရိုက်ခဲ့လျှင်)
    if 'quick_buy_item' in context.user_data:
        item = context.user_data.pop('quick_buy_item')
        price = context.user_data.pop('quick_buy_price')
        context.user_data.update({'order_qty': item, 'order_price': price, 'order_item': b_name})
        kb = [[InlineKeyboardButton("✅ ဟုတ်တယ်", callback_data="conf_yes"), 
               InlineKeyboardButton("❌ မဟုတ်ဘူး", callback_data="conf_no")]]
        await query.edit_message_text(f"🛒 **အတည်ပြုရန်**\nပစ္စည်း: {b_name} ({item})\nကျသင့်ငွေ: {price:,} ks\n\nမှန်ကန်ပါသလားရှင်?", reply_markup=InlineKeyboardMarkup(kb))
        return CONFIRM_ORDER

    if p_type == "game":
        await query.edit_message_text("ဝယ်ယူမည့် ပမာဏကို ဂဏန်းဖြင့် ပို့ပေးပါရှင်။ (ဥပမာ- 720)")
        return WAIT_QTY
    else:
        await query.edit_message_text("ဝယ်ယူမည့် အမျိုးအစား အမည်ကို ပို့ပေးပါရှင်။ (ဥပမာ- 1 month)")
        return WAIT_QTY # Premium အတွက်လည်း handle_quantity က အမည်တူ ရှာပေးပါလိမ့်မည်
...
def init_db():
    try:
        ...
        # ၁။ Settings Table
        cursor.execute("CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, value TEXT, chat_id BIGINT)")
        ...
        # ၃။ Learning Cache Table (PT အလိုက် သီးသန့်မှတ်ဉာဏ်)
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS learning_cache (
                id SERIAL PRIMARY KEY,
                btn_id INTEGER REFERENCES buttons(id) ON DELETE CASCADE,
                raw_input TEXT,
                resolved_name TEXT,
                UNIQUE(btn_id, raw_input)
            )
        """)
        ...
        cursor.close()
        conn.close()
        print("✅ Database Tables အားလုံး အဆင့်မြင့် Logic များဖြင့် အဆင်သင့်ဖြစ်ပါပြီ။")
    except Exception as e:
...
async def handle_quantity(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    res = db_query("SELECT parsed_data, name, product_type FROM buttons WHERE id=%s", (btn_id,), fetch=True)
    
    if not res or not res[0][0]:
        await update.message.reply_text("❌ စျေးနှုန်းစာရင်း မရှိသေးပါ။")
        return ConversationHandler.END

    p_data, b_name, p_type = res[0]
    target = None

    if p_type == "game":
        # Game ဆိုလျှင် ဂဏန်းကိုပဲ ဆွဲထုတ်မည်
        qty_num = re.findall(r'\d+', clean_and_normalize(user_input))
        if qty_num: target = qty_num[0]
    else:
        # Premium ဆိုလျှင် User ရိုက်သည့် စာသားအတိုင်း ရှာမည် (ဥပမာ- "1 month")
        target = user_input.strip()

    price = p_data.get(target)

    if price:
        context.user_data.update({'order_qty': target, 'order_price': price, 'order_item': b_name})
        kb = [[InlineKeyboardButton("✅ ဟုတ်တယ်", callback_data="conf_yes"), 
               InlineKeyboardButton("❌ မဟုတ်ဘူး", callback_data="conf_no")]]
        await update.message.reply_text(f"🛒 **အတည်ပြုရန်**\nပစ္စည်း: {b_name} ({target})\nကျသင့်ငွေ: {price:,} ks\n\nမှန်ကန်ပါသလားရှင်?", 
                                       reply_markup=InlineKeyboardMarkup(kb))
        return CONFIRM_ORDER
    else:
        await update.message.reply_text("❌ ထိုပမာဏ/အမျိုးအစားကို ဝယ်ယူ၍မရပါ။ စာရင်းထဲရှိသည့်အတိုင်း ပြန်ရိုက်ပေးပါရှင်။")
        return WAIT_QTY
...
async def handle_user_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ၁။ ခလုတ်နာမည်နဲ့ တိုက်ရိုက်တူရင် (Navigation Logic)
    btn_res = db_query("SELECT id, display_text, content_msg_id, content_chat_id FROM buttons WHERE name=%s", (user_input,), fetch=True)
    if btn_res:
        btn_id, display_text, c_msg, c_chat = btn_res[0]
        if c_msg:
            context.user_data['last_pt_id'] = btn_id
            await context.bot.copy_message(chat_id=update.effective_chat.id, from_chat_id=c_chat, message_id=c_msg)
            return await show_buy_button(update, context, btn_id)

    # ၂။ စာရိုက်လာပါက Context-Aware Learning စနစ်သုံးခြင်း
    if last_pt_id:
        cache_res = db_query("SELECT resolved_name FROM learning_cache WHERE btn_id=%s AND raw_input=%s", 
                             (last_pt_id, user_input.lower()), fetch=True)
        
        target_name = None
        if cache_res:
            target_name = cache_res[0][0]
        else:
            pt_info = db_query("SELECT name, parsed_data FROM buttons WHERE id=%s", (last_pt_id,), fetch=True)
            if pt_info and pt_info[0][1]:
                prompt = f"In the price list of '{pt_info[0][0]}', what item does '{user_input}' refer to? List of items: {list(pt_info[0][1].keys())}. Return ONLY the matched item name or 'None'."
                ai_resp = await model.generate_content_async(prompt)
                match = ai_resp.text.strip()
                if match in pt_info[0][1]:
                    target_name = match
                    db_query("INSERT INTO learning_cache (btn_id, raw_input, resolved_name) VALUES (%s, %s, %s) ON CONFLICT DO NOTHING", 
                             (last_pt_id, user_input.lower(), target_name))

        if target_name:
            p_data = db_query("SELECT parsed_data FROM buttons WHERE id=%s", (last_pt_id,), fetch=True)[0][0]
            price = p_data.get(target_name)
            if price:
                context.user_data['quick_buy_item'] = target_name # တိုက်ရိုက်ဝယ်ရန် သိမ်းခြင်း
                context.user_data['quick_buy_price'] = price
                kb = [[InlineKeyboardButton(f"🛒 {target_name} ကို ဝယ်ယူမည်", callback_data=f"buy_{last_pt_id}")]]
                await update.message.reply_text(f"💰 စျေးနှုန်း: {price:,} ks\n\nဝယ်ယူရန် အောက်ကခလုတ်ကို နှိပ်ပါရှင် -", 
                                               reply_markup=InlineKeyboardMarkup(kb))
                return
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            CONFIRM_PT_SAVE: [
                CallbackQueryHandler(confirm_pt_save, pattern="^confirm_pt_save"),
                CallbackQueryHandler(handle_category, pattern="^act_edit") # ပြန်ပြင်မည်ဆိုလျှင်
            ],
