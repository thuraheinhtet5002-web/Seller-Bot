...
async def save_pt_content(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ၁။ Content စုဆောင်းခြင်း (Photo သို့မဟုတ် Text)
    if msg.photo:
    ...
    # AI ကို Emoji တွေ ဖယ်ပြီး JSON ထုတ်ခိုင်းမည့် Prompt
    prompt = """
    Extract price data. Ignore all emojis, symbols, and units like 'ks'.
    Format Example: '172 = 9950' -> Quantity: 172, Price: 9950.
    Return ONLY a raw JSON object:
    {
      "category": "game" or "premium",
      "main_item": "Item Name",
      "items": { "quantity": price_integer }
    }
    ...
    try:
        ...
        # JSON Block ကိုပဲ ရှာပြီး ဆွဲထုတ်ခြင်း
        json_match = re.search(r'\{.*\}', res_text, re.DOTALL)
        if json_match:
            parsed_data = json.loads(json_match.group(0))
        else:
            parsed_data = json.loads(res_text.replace('```json', '').replace('```', '').strip())

        # Admin Preview ပြခြင်း
        ...
        await msg.reply_text(preview_text + "\n\nမှန်ကန်ပါသလားဗျ?", reply_markup=InlineKeyboardMarkup(kb))
